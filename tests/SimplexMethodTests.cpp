...
TEST(SimplexMethodTests, TableauInitialization) {
  {
    ...
    auto [tableau, basic_vars] = solver.initialize_tableau(bfs);
    ...
    ASSERT_EQ(basic_vars, (std::vector<size_t>{2, 3, 4}));
  }

  {
    ...
    ASSERT_EQ(expected, tableau);
    ASSERT_EQ(basic_vars, (std::vector<size_t>{2, 3}));
  }
}
...
TEST(SimplexMethodTests, SimplexMethodStartingInSolution) {
  ...
  SimplexMethod solver(A, b, c);
  auto solution = solver.solve_from(bfs);
